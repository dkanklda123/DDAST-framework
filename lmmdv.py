...
class LMMD_loss(nn.Module):
    def __init__(self, class_num=5, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    ...
    def convert_to_onehot(self, sca_label, class_num=5):
        return np.eye(class_num)[sca_label]

    def cal_weight(self, s_label, t_label, batch_size=64, class_num=5):
        batch_size = s_label.size()[0]
        s_sca_label = s_label.cpu().data.numpy()
        s_vec_label = self.convert_to_onehot(s_sca_label.astype(int), class_num=self.class_num)
